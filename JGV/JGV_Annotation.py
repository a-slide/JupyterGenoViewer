...
class Annotation(object):
    ...
    def __init__ (self, fp, name=None, verbose=False, ref_list=[]):
        ...
        * ref_list
        ...
        if self.ext == "gtf":
            if self.verbose: jprint("Use GTF parser to parse annotations in", self.name)
            self.df = self._gtf_parser(fp, ref_list)
        elif self.ext == "gff3":
            if self.verbose: jprint("Use GFF3 parser to parse annotations in ", self.name)
            self.df = self._gff3_parser(fp, ref_list)
        elif self.ext == "bed":
            if self.verbose: jprint("Use BED parser to parse annotations in ", self.name)
        ...
        if self.verbose:
            jprint ("\tFound {} features in {} reference sequences".format( self.feature_count, self.refid_count))
    ...
    def interval_features (self, refid, start, end, feature_types=None, max_features_per_type=None):
        ...
        * max_features_per_type
        ...
        if feature_types and self.ext == "bed":
            if self.verbose: jprint ("Incompatible options. Bed files do not allow to identify feature_type")
            return pd.DataFrame(columns=["refid","start","end","strand","ID","type"])

        # Select the refid and coordinates
        df = self.df[(self.df["refid"] == refid)&(self.df["end"] > start)&(self.df["start"] < end)]
        if df.empty:
            if self.verbose: jprint("No feature found in the requested interval")
            return pd.DataFrame(columns=["refid","start","end","strand","ID","type"])
        ...
        for type_name, type_df in df.groupby("type"):
            ...
            if not feature_types or type_name in feature_types:
                ...
                if sdf.empty:
                    if self.verbose: jprint("No feature of type {} found in the requested interval".format(type_name))
