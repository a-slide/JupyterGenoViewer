...
class Alignment(object):
    ...
    def __init__ (self, fp, name=None, verbose=False, min_coverage=5, ref_list=[], output_bed=False):
         ...
         * fp
             ...
             coverage value (Much faster than from a Bam/Sam file, can be gzipped).
        ...
        * output_bed
        ...
        if self.verbose:
            print ("\tTotal base coverage {} in {} reference sequences".format( self.nbases, self.refid_count))
    ...
    def _bed_parser (self, fp):
        ...
        with open_fun(fp, open_mode) as fin:
            if self.verbose: print("\tExtract base coverage data")
            for line in fin:
                sl = line[0:-1].split("\t")
                refid = sl[0]
                if not refid in d:
