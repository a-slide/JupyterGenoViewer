...
class Alignment(object):
    ...
    def __init__ (self, fp, name=None, verbose=False):
        """
         * fp
             A standard bam file already SORTED BY COORDINATES (http://samtools.sourceforge.net/SAM1.pdf) containing
             aligned reads. Ideally the file would already be indexed with samtools index. If not the program will
             index the file (can take time)
        *  name
    ...
    def seqid_read_count (self):
        """
        Return a dataframe containing 3 colums ("length", "read_count", "read_density") for all seq_id reference
    ...
    def interval_coverage (self, seqid, start, end, n_step=500, mode="auto"):
        """
       ...
       * n_step
            ...
            will be faster. The default value is a decent compromise between speed and precision [ DEFAULT: 500 ]
        * mode
            Mode to parse and represent the alignment track ("auto"|"read_count"|"base_coverage").
            The "read_count" is fast for large regions, but do not account for spliced reads. Automatically selected in
            "auto" mode for intervals larger than 5 Mb.
            The "base_coverage" mode compute the median base coverage per step and is usually faster for low coverage
            and small intervals. The resolution is far better than the "read_count" mode. Automatically selected in
            "auto" mode for intervals smaller than 5 Mb."""
        ...
        if seqid not in self.seqid_list:
            if self.verbose:
                print("Seqid ({}) not found in the list for this alignment {}".format(seqid, self.name))
            return pd.DataFrame()
